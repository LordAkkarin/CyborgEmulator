 ...
 */
@Opcode (0x5000)
public class EqualsRegisterOpcode implements IOpcode<Chip8Emulator> {
	...
	public boolean execute (Chip8Emulator emulator, long opcode) throws OpcodeException {
		...
		try {
			if (emulator.getProcessor ().getRegister ().get (registerA) == emulator.getProcessor ().getRegister ().get (registerB)) emulator.getProcessor ().skipInstruction ();
