...
/**
 * 0xFX55: Copies all register values from register 0 to register X into memory.
 * @author Johannes Donath <johannesd@evil-co.com>
 * @copyright Copyright (C) 2014 Evil-Co <http://www.evil-co.com>
 */
@Opcode (0xF055)
public class CopyRegisterOpcode implements IOpcode<Chip8Emulator> {
	...
	public boolean execute (Chip8Emulator emulator, long opcode) throws OpcodeException {
		...
		try {
			for (short i = 0; i < register; i++) {
				emulator.getMemory ().set ((emulator.getProcessor ().getIndexRegister () + i), emulator.getProcessor ().getRegister ().get (i));
			}
